...
import { anyType, Parameters } from "./interfaces";
...
export function setParam<T>(name: string, val: T) {
  anyType(settings)[name] = val;
  try {
...
function firstDefined<T>(...vals: unknown[]): T | undefined {
...
export function setDefaults(parameters: Parameters | undefined) {
  loadSettings();
  ...
  params.activeAlphaColor = HexToRGBA(params.activeColor, def(params.activeColorAlpha, 1));

  params.activeFillColor = settings.activeFillColor || params.activeFillColor || "#000000";
  params.activeFillColorAlpha = firstDefined(settings.activeFillColorAlpha, params.activeFillColorAlpha, 0.0);
  params.activeFillAlphaColor = HexToRGBA(params.activeFillColor, def(params.activeFillColorAlpha, 1));
  ...
  params.defaultPrimitiveShadowOn = firstDefined(settings.defaultPrimitiveShadowOn, params.defaultPrimitiveShadowOn, true);

  params.defaultArrowLength = settings.defaultArrowLength || params.defaultArrowLength || 32;
  params.defaultEraserWidth = firstDefined(settings.defaultEraserWidth, params.defaultEraserWidth, 5);
  ...
  params.defaultFontItalic = firstDefined(settings.defaultFontItalic, params.defaultFontItalic, false);
  ...
  params.backgroundFillColorAlpha = params.backplateImgUrl ? 0 : firstDefined(settings.backgroundFillColorAlpha, params.backgroundFillColorAlpha, 1.0);
  params.backgroundFillAlphaColor = HexToRGBA(params.backgroundFillColor, params.backgroundFillColorAlpha || 1.0);
  ...
  params.textStrokeAlphaColor = HexToRGBA(params.textStrokeColor, params.textStrokeColorAlpha || 1.0);
  ...
  params.defaultSize = params.defaultSize || "fill"; // TODO defaults
  ...
  if (params.translation) {
    ...
    Translation.get().addTranslation(name, params.translation);
