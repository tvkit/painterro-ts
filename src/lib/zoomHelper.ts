...
export default class ZoomHelper {
  ...
  private gridColor: ImageData;
  private gridColorRed: ImageData;
  private captW: number;
  private middle: number;
  private periodW: number;
  private fullW: number;
  private halfFullW: number;
  ...
  constructor(main: Main) {
    ...
    this.zomer.setAttribute("width", this.fullW.toString());
    this.zomer.setAttribute("height", this.fullW.toString());
  ...
  handleMouseMove(e: MouseEvent) {
    if (this.main.colorPicker.choosing && !e.altKey) {
      ...
      const cord = [e.clientX - this.main.elLeft() + this.main.scroller.scrollLeft, e.clientY - this.main.elTop() + this.main.scroller.scrollTop];

      let x = (cord[0] || 0) * scale;
      x = x < 1 ? 1 : x;
      x = x > this.main.size.w - 1 ? this.main.size.w - 1 : x;
      let y = (cord[1] || 0) * scale;
      y = y < 1 ? 1 : y;
